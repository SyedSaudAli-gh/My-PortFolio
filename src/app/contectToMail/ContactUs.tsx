    ...
    <>
      ...
      {isLoading && (
        <div className="fixed inset-0 z-[9999] grid place-items-center bg-black/40 backdrop-blur-sm">
          <Loader>
            <span className="text-black dark:text-white">Sending your message…</span>
          </Loader>
        </div>
      )}
      ...
      <div
        ...
        <div className="mx-auto relative flex flex-col md:flex-row w-full max-w-[980px] min-h-[700px] items-stretch overflow-hidden rounded-2xl bg-white/10 border border-white/20 shadow-[0_0_30px_rgba(0,0,0,0.06)] after:absolute after:content-[''] after:w-[80%] after:h-[80%] after:right-[-35%] after:top-[10%] after:rounded-full after:z-[-1] after:bg-[radial-gradient(circle,_rgba(34,211,238,0.22)_0%,_rgba(168,85,247,0.22)_55%,_rgba(236,72,153,0.22)_100%)]">
          ...
          {/* Left - Form Fields */}
          <div className="md:w-2/3 w-full backdrop-blur-sm">
            <form
              onSubmit={handleSubmit}
              className="relative flex h-full w-full flex-col justify-center py-8 md:py-10"
              aria-busy={isLoading}
            >


              {["name", "email", "company", "jobTitle", "subject"].map((field) => (
                <div className="relative" key={field}>
                  <input
                    className="peer bg-transparent outline-none border border-transparent rounded-md p-[10px] my-[10px] mx-auto w-[80%] block text-[#1f2b44] dark:text-[#cfd8ea] font-medium text-[1.05em] shadow-[inset_4px_4px_4px_rgba(165,163,163,0.22),_4px_4px_4px_rgba(218,218,218,0.08)] focus:ring-2 focus:ring-purple-400/40"
                  ...
                  <label
                    htmlFor={field}
                    className="pointer-events-none absolute left-[15%] top-[37%] text-gray-400 transition-all duration-300 peer-focus:-translate-y-[120%] peer-focus:scale-90 peer-[&:not(:placeholder-shown)]:-translate-y-[120%] peer-[&:not(:placeholder-shown)]:scale-90"
                  >
              ...
              <div className="relative">
                <textarea
                  className="peer bg-transparent outline-none border border-transparent rounded-md p-[10px] my-[10px] mx-auto w-[80%] block text-[#1f2b44] dark:text-[#cfd8ea] font-medium text-[1.02em] shadow-[inset_4px_4px_4px_rgba(165,163,163,0.22),_4px_4px_4px_rgba(218,218,218,0.08)] h-32 resize-none focus:ring-2 focus:ring-purple-400/40"
                  id="message"
                  name="message"
                  required
                ...
                <label
                  htmlFor="message"
                  className="pointer-events-none absolute left-[15%] top-[20%] text-gray-500 transition-all duration-300 peer-focus:-translate-y-[160%] peer-focus:scale-90 peer-[&:not(:placeholder-shown)]:-translate-y-[160%] peer-[&:not(:placeholder-shown)]:scale-90"
                >
                  Message
                </label>
              </div>
              ...
              <div className="relative w-[80%] mx-auto my-[10px]">
                <label htmlFor="file" className="block text-sm text-gray-500 mb-1">
                  Attach File <span className="text-xs text-gray-400">(optional)</span>
                </label>
                <input
                  type="file"
                  id="file"
                  ...
                  className="w-full bg-transparent text-sm text-[#1f2b44] dark:text-[#cfd8ea] file:mr-4 file:py-2 file:px-4
                    file:rounded file:border-0 file:text-sm file:font-semibold
                  file:bg-gray-700 file:text-white                      // Light mode style
                  dark:file:bg-white dark:file:text-black            
                    hover:file:brightness-110 transition-all duration-300"
                />
                {file && (
                  <p className="text-xs text-green-500 mt-1">Selected file: {file.name}</p>
                )}
              </div>
              ...
              <div className="relative">
                <button
                  type="submit"
                  disabled={isLoading}
                  className="mx-auto block w-[80%] rounded-md p-[10px] my-[10px] text-white font-semibold
                    bg-gradient-to-r from-cyan-400 via-purple-400 to-pink-400
                    shadow-[0_10px_25px_rgba(168,85,247,0.25)]
                    hover:shadow-[0_12px_30px_rgba(168,85,247,0.35)]
              ...
              {/* Error UI */}
              {error && (
                <p className="w-[80%] mx-auto mt-1 text-red-600 text-sm text-center">
                  {error}
                </p>
              )}
            </form>
          ...
          <div className="md:w-1/3 w-0 md:block hidden">
